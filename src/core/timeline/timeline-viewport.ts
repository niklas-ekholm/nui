...
function attachRangePan(options: RangePanOptions): void {
	const { surface, getRange, getViewportWidth, onPreview, onCommit, onPanStart } =
		options;
...
export function attachTimelineViewport(
	...
	const axisTarget =
		chart?.querySelector<HTMLElement>(".nui-timeline-body") ?? chart;
	if (root && axisTarget) {
		let lastAxisWidth = 0;
		const axisResizeObserver = new ResizeObserver((entries) => {
			const width = entries[0]?.contentRect.width ?? 0;
			if (width <= 0 || width === lastAxisWidth) return;
			lastAxisWidth = width;
			requestAnimationFrame(() => {
				updateTimelineRangePreview(root, liveRangeStart, liveRangeEnd);
			});
		});
		axisResizeObserver.observe(axisTarget);
	}
}
