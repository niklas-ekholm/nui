...
import { addDays, dateRangesOverlap, daysBetween, formatDayOfMonth, formatDisplayDate, formatIsoDate, shiftRangeStartToDate, startOfDay } from "../parse/dates";
...
import { shouldDeferTimelineDropRender } from "./post-drop-visibility";
...
export function renderTimeline(
	...
	if (shouldDeferTimelineDropRender(container, items)) return;
...
function renderTimelineContent(
	...
	const stickyHeader = el("div", "nui-timeline-sticky-header");
	...
	for (const row of layout.rows) {
		...
		if (row.item.color) {
			titleTextEl.dataset.nuiEventColor = row.item.color;
	...
	attachTimelineSelection({
		...
		onDatesBatchChange: options.onDatesBatchChange,
		...
		onSetItemColor: options.onSetItemColor,
