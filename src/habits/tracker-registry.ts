...
import type { MonthTrackerBasesView } from "../views/month-tracker-bases-view";
...
const monthTrackerViews = new Set<MonthTrackerBasesView>();
...
export function registerMonthTrackerView(view: MonthTrackerBasesView): void {
	monthTrackerViews.add(view);
}

export function unregisterMonthTrackerView(view: MonthTrackerBasesView): void {
	monthTrackerViews.delete(view);
}
...
export function refreshAllTrackerViews(): void {
	...
	for (const view of monthTrackerViews) {
		view.refreshAfterHabitChange();
	}
}
