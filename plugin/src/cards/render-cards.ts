...
export interface RenderCardsOptions {
	...
	entryTitle?: (entry: BasesEntry) => string;
	listPrefix?: string;
...
function renderCard(
	...
	const title = options.entryTitle?.(entry) ?? defaultEntryTitle(entry);
	...
	if (imageSrc) {
		...
		cover.setAttr("aria-label", title);
	}
	...
	for (const propertyId of options.order) {
		...
		renderProperty(body, entry, propertyId);
	}
...
function defaultEntryTitle(entry: BasesEntry): string {
