...
const MARQUEE_PREVIEW_CLASS = "is-marquee-preview";
...
function rowsIntersectingMarquee(
	...
	for (const row of Array.from(
		...
		const hitEl = elements.eventEl ?? elements.bar;
		const hitRect = hitEl.getBoundingClientRect();
		const marqueeHit = normalizeRect(
			hitRect.left,
			hitRect.top,
			hitRect.right,
			hitRect.bottom,
		...
		if (rectsIntersect(marquee, marqueeHit)) {
			const id = row.dataset.itemId;
			if (id) hits.push(id);
		}
	}
...
function clearMarqueePreview(body: HTMLElement): void {
	for (const row of Array.from(
		body.querySelectorAll<HTMLElement>(`.${MARQUEE_PREVIEW_CLASS}`),
	)) {
...
function updateMarqueePreview(
	...
	const hits = new Set(rowsIntersectingMarquee(body, marquee));
	for (const row of Array.from(
		body.querySelectorAll<HTMLElement>(".nui-timeline-row[data-item-id]"),
	)) {
		const id = row.dataset.itemId ?? "";
...
function isEventGapTarget(target: EventTarget | null): boolean {
	if (!(target instanceof HTMLElement)) return false;
	if (!target.closest(".nui-timeline-event")) return false;
	if (barFromTarget(target)) return false;
	if (isTitleTarget(target)) return false;
	if (isHandleTarget(target)) return false;
	if (target.closest(".nui-timeline-bar-title-text-editing")) return false;
	return true;
}
...
export function attachTimelineSelection(options: TimelineSelectionOptions): void {
	...
	const onKeyDown = (event: KeyboardEvent) => {
		...
		if (event.code === "Escape" && dragActive) {
	...
	const onBodyPointerDown = (event: PointerEvent) => {
		...
		const selectionBeforeDrag = cloneSelection(selectedIds);
		...
		const eventGapClick = isEventGapTarget(event.target);
		let barRow = bar?.closest<HTMLElement>(".nui-timeline-row[data-item-id]");
		if (!barRow && eventGapClick && event.target instanceof HTMLElement) {
			barRow = event.target.closest<HTMLElement>(
				".nui-timeline-row[data-item-id]",
			);
		}
		const barId = barRow?.dataset.itemId;
		const isBarInteraction = !!(bar || eventGapClick);
		...
		} else if (editable && onDatesChange && isBarInteraction && barId) {
			mode = selectedIds.has(barId) ? "move" : "bar";
		} else if (isBarInteraction && barId) {
			mode = "bar";
		}
		...
		const populateMoveStarts = () => {
			const moveIds = expandMoveIdsWithSubtree(selectedIds, itemsById);
			commitSelection(root, selectedIds, moveIds, onSelectionChange);
			moveStarts.clear();
			for (const id of moveIds) {
				const row = body.querySelector<HTMLElement>(
					`.nui-timeline-row[data-item-id="${CSS.escape(id)}"]`,
				);
				const dates = row ? readTimelineRowDates(row) : null;
				if (dates) {
				...
				if (item) {
					moveStarts.set(id, { start: item.start, end: item.end });
				}
			}
			referenceTrack =
		...
		trackPointerDrag({
			...
			onMove: (moveEvent) => {
				...
				updateMarqueePreview(body, marquee);
			},
			onEnd: (upEvent) => {
				finishDrag();
				...
				if (mode === "move" && moved && (onDatesBatchChange || onDatesChange)) {
					...
					for (const [id, original] of moveStarts) {
						...
						const next = row
							? readTimelineRowDates(row)
							: {
									start: addDays(original.start, lastDayDelta),
									end: addDays(original.end, lastDayDelta),
								};
						if (!next) continue;
					...
					if (updates.length > 0) {
						...
						if (onDatesBatchChange) {
							onDatesBatchChange(updates);
						} else {
							for (const update of updates) {
								onDatesChange?.(update.item, update.start, update.end);
							}
						}
					}
					window.setTimeout(() => {
	...
	const onBodyContextMenu = (event: MouseEvent) => {
		...
		const isEventGap = isEventGapTarget(event.target);
		const isTitle = isTitleTarget(event.target);

		if (isBar || isEventGap || isTitle) {
			if (
				!onDeleteItems &&
				!onDuplicateItems &&
				!onTurnIntoProjectFolder &&
