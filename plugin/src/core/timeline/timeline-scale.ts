...
export const MIN_PX_PER_DAY_FOR_DAY_GRID_LINES = 60;
...
export function buildDayGridTicks(
	_rangeStart: Date,
	totalDays: number,
): TimelineTick[] {
	const ticks: TimelineTick[] = [];
	...
	for (let offset = 1; offset < dayCount; offset += 1) {
		pushGridTick(ticks, totalDays, offset);
	}

	return ticks;
}

export function shouldShowDayGridLines(
	totalDays: number,
	chartWidthPx: number,
	minPxPerDay: number = MIN_PX_PER_DAY_FOR_DAY_GRID_LINES,
): boolean {
