...
import { getFolderIndexFolderName } from "../navigation/folder-index-path";
...
const COLOR_PROPERTY: BasesPropertyId = "note.color";

function titleFromPath(path: string): string {
	const folderName = getFolderIndexFolderName(path);
...
function readColor(entry: BasesEntry): string | undefined {
	return (
		normalizeHexColor(valueToString(entry.getValue(COLOR_PROPERTY))) ??
		undefined
	);
}
