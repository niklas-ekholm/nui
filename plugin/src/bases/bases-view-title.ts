...
function findMarkdownViewContainer(el: HTMLElement): HTMLElement | null {
	let node: HTMLElement | null = el;
	while (node) {
		if (node.querySelector(".view-content, .cm-scroller")) return node;
		node = node.parentElement;
	}
	return null;
}

export function applyTimelineBasesChrome(
	...
	const embedRoot = findEmbedRoot(containerEl);
	if (!embedRoot) return;

	const parsed = resolveTimelineEmbedPipes(app, containerEl) ?? { rawTokens: [] };
	if (!shouldEmbedBeWide(parsed, true)) return;

	embedRoot.setAttribute("data-nui-embed-wide", "");
	const viewContainer = findMarkdownViewContainer(embedRoot);
	if (viewContainer) syncPaneWidthInElement(viewContainer);
}
