#!/usr/bin/env bash
#
# Fetch a MiniNUI release and install it into an Obsidian vault, in one command.
#
#   curl -fsSL https://raw.githubusercontent.com/niklas-ekholm/nui/main/scripts/install-remote-mini.sh | bash -s -- /path/to/vault
#
...
# editor tools, and folder-index navigation, generated from the same source at
# release time. It installs as plugin `mininui` and theme `MiniNUI`, so it does
# not touch an existing NUI install — enable only one of the two plugins.
#
...
usage() {
    cat <<'EOF'
Fetch a MiniNUI release and install it into an Obsidian vault.

  install-remote-mini.sh [options] [/path/to/vault]
  install-remote-mini.sh [options] --all

With no vault path (and without --all), installs into the current directory.

Options:
  --version <tag>  install a specific release, e.g. v0.2.7 (default: latest)
  --plugin-only    install the plugin, skip the theme
  --theme-only     install the theme, skip the plugin
  --no-verify      skip checksum verification
  -h, --help       this message
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --version)
            [[ $# -ge 2 ]] || die "--version needs a tag, e.g. --version v0.2.7"
            VERSION="$2"; shift 2 ;;
...
if [[ -z "${VERSION}" ]]; then
    ...
    [[ -n "${VERSION}" ]] || die "could not resolve the latest release.
       GitHub's API rate-limits unauthenticated requests; pass --version v0.2.7 instead."
fi
...
for component in "${COMPONENTS[@]}"; do
    info "  fetching mininui-${component}-${NUMBER}.zip"
    fetch "mininui-${component}-${NUMBER}.zip"
done
...
# must not share one: the plugin ships mininui/manifest.json and the theme ships
# MiniNUI/manifest.json, which collide on a case-insensitive filesystem — the
# default on macOS.
for component in "${COMPONENTS[@]}"; do
    dir="${TMP}/${component}"
    mkdir -p "${dir}"
    ...
    case "${UNPACK}" in
        unzip) unzip -q "${TMP}/mininui-${component}-${NUMBER}.zip" -d "${dir}" ;;
        bsdtar) tar -xf "${TMP}/mininui-${component}-${NUMBER}.zip" -C "${dir}" ;;
    esac

    [[ -f "${dir}/install.sh" ]] \
        || die "mininui-${component}-${NUMBER}.zip contains no install.sh"
