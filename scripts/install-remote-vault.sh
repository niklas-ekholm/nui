...
Options:
  --version <tag>  install a specific release, e.g. v0.2.3 (default: latest)
  --no-verify      skip checksum verification
  -h, --help       this message
EOF
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --version)
            [[ $# -ge 2 ]] || die "--version needs a tag, e.g. --version v0.2.3"
            VERSION="$2"; shift 2 ;;
...
if [[ -z "${VERSION}" ]]; then
    ...
    [[ -n "${VERSION}" ]] || die "could not resolve the latest release.
       GitHub's API rate-limits unauthenticated requests; pass --version v0.2.3 instead."
fi
